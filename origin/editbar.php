...
$source = mysql_query("SELECT * FROM alc_items WHERE id='$param_id'");
while($rowx = mysql_fetch_array($source)){
$id = $rowx['id'];
$cat_id = $rowx['alccat_id'];

$name_ua = $rowx['alc_name_ua'];
$str_ua = $rowx['structure_ua'];
$ci_ua = $rowx['w_ci_UA'];

$name_en = $rowx['alc_name_en'];
$str_en = $rowx['structure_en'];
$ci_en = $rowx['w_ci_EN'];

$name_ru = $rowx['alc_name_ru'];
...
<h1>Редактировать напиток</h1>
	...
	<form id="saveeat" method="post" action="admin_files/edit_one_bar.php">
...
$source = mysql_query("SELECT * FROM alc_cats WHERE id='$cat'");
while($rowx = mysql_fetch_array($source)){
$catid = $rowx['id'];
$catname = $rowx['alccat_ru'];
}
