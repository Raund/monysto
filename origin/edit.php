...
$source = mysql_query("SELECT * FROM menu_items WHERE id='$param_id'");
while($rowx = mysql_fetch_array($source)){
$id = $rowx['id'];
$cat_id = $rowx['cat_id'];

$name_ua = $rowx['food_name_ua'];
$str_ua = $rowx['structure_ua'];
$ci_ua = $rowx['w_ci_UA'];

$name_en = $rowx['food_name_en'];
$str_en = $rowx['structure_en'];
$ci_en = $rowx['w_ci_EN'];

$name_ru = $rowx['food_name_ru'];
...
<h1>Редактировать блюдо</h1>
	...
	<form id="saveeat" method="post" action="admin_files/edit_one_eat.php">
...
$source = mysql_query("SELECT * FROM menu_cats WHERE id='$cat'");
while($rowx = mysql_fetch_array($source)){
$catid = $rowx['id'];
$catname = $rowx['cat_ru'];
}
...
unset($cat);	
	...
	<div class="editblock-names" >
		...
		<div><input class="editinp2" type="text" name="str_ru" value="<?print $str_ru?>" placeholder="(ингредиенты)"></div>
	...
	<br><a class="back" id="backlink" href="/origin/root.php?param=eat">Вернуться</a>
