...
(function($, window, undefined) {
	...
	$.widget('comiseo.daterangepicker', {
		...
		options: {
			...
			altFormat: 'dd.mm.yy', // submitted date format - inside JSON {"start":"...","end":"..."}
...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildCalendar(classnameContext, options) {
		...
		function onSelectDay(dateText, instance) {
			...
			if (!range.start || range.end) { // start not set, or both already set
				...
				range.start = selectedDate;
				document.getElementById('getdate').click();
			} else {
	...
	function buildButtonPanel(classnameContext, options, handlers) {
		...
		function init() {
			applyButton = $('<button id="getdate" type="button" class="ui-priority-primary"></button>')
				.text(options.applyButtonText)
				.button();
			cancelButton = $('<button id="getdate" type="button" class="ui-priority-secondary"></button>')
